...
let medicine = document.getElementById("container");

let generateMedicine = () => {
    return (medicine.innerHTML = medicineData.map((x) => {
        return `<a href="">
...
let singleProductPage = (id) => {
    let selectedItem = id;
    let search = medicineData.find((x) => x.id === selectedItem);
